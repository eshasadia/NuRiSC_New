...
#SE Operation: Eliminate Redundant Information and Aggregate Effective Information
...
class PatchEncoder(layers.Layer):
    ...
    def get_config(self):  # 在有自定义网络层时，需要保存模型时，重写get_config函数
        config = {"num_patches": self.num_patches, "projection": self.projection,
...
def transformer(x):
    ...
    encoded_patches = Reshape((int(image_size / patch_size), int(image_size / patch_size), projection_dim))(
        encoded_patches)
...
def encoder1(inputs):
    ...
    model = VGG19(include_top=True, weights='imagenet', input_tensor=inputs)
